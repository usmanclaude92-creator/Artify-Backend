 ...
 *
 * Adapted: the source repo's `NotificationPreference` was one row per user
 * with `emailEnabled`/`smsEnabled` booleans. This repo's own Phase 1
 * `NotificationPreference` (unchanged here) is one row per
 * (user, channel, notificationType) — finer-grained, matching every other
 * channel/type pair a user might individually opt out of. The suppression
 * check below queries that existing shape instead.
 */
...
export class NotificationEngine {
  ...
  public async dispatchNotification(params: {
    ...
    // Check user preference if userId provided and channel is external (EMAIL/SMS)
   ...
   */
  public async listNotifications(params: { organizationId: string; userId?: string; page?: number; limit?: number }) {
