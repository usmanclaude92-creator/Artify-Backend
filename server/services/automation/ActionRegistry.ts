 ...
 * Controlled Business Action Registry — this subsystem's own governed
 * "tool registry" for BUSINESS_ACTION workflow steps (docs/AUTOMATION_ARCHITECTURE.md),
 * separate from and no less strict than the main AI Control Center's
 * (server/ai/toolRegistry.ts) — each action still declares its own
 * requiredPermission/riskLevel/requiresApproval and is only ever invoked
 * through `executeAction` below, never directly.
 *
 * Adapted: two of the source repo's field names didn't match this repo's
 * actual schema (`Lead.assignedTo`, not `assignedUserId`;
 * `Client.accountManager`, not `accountManagerId` — both pre-existing
 * Phase 5 fields, unrelated to this import) — fixed in `assign_user` below.
 */
...
export class ActionRegistry {
  ...
  private registerStandardActions(): void {
    ...
    this.registerAction({
      ...
      outputSchema: z.object({ taskId: z.string(), title: z.string(), status: z.string() }),
    ...
    this.registerAction({
      ...
      inputSchema: z.object({
        ...
        status: z.enum(["PROSPECT", "ACTIVE", "INACTIVE", "SUSPENDED", "ARCHIVED"]).optional(),
        notes: z.string().optional(),
      }),
      outputSchema: z.object({ clientId: z.string(), updated: z.boolean() }),
      execute: async (input, context) => {
        const client = await prisma.client.findFirst({ where: { id: input.clientId, organizationId: context.organizationId } });
        if (!client) {
    ...
    this.registerAction({
      ...
      outputSchema: z.object({ notificationId: z.string(), delivered: z.boolean() }),
      execute: async (input, context) => {
    ...
    this.registerAction({
      ...
      outputSchema: z.object({ entityId: z.string(), assignedUserId: z.string(), success: z.boolean() }),
    ...
    this.registerAction({
      ...
      requiresApproval: false,
      requiresAudit: true,
      inputSchema: z.object({ workflowId: z.string().min(1), status: z.enum(["ACTIVE", "PAUSED", "ARCHIVED"]) }),
      outputSchema: z.object({ workflowId: z.string(), newStatus: z.string() }),
      execute: async (input, _context) => {
        const updated = await prisma.automationWorkflow.update({ where: { id: input.workflowId }, data: { status: input.status as any } });
    ...
    this.registerAction({
      ...
      requiresApproval: true,
      requiresAudit: true,
      inputSchema: z.object({ recipientUserId: z.string().min(1), title: z.string().min(1), message: z.string().min(1) }),
