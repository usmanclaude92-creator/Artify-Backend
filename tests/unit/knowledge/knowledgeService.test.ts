/**
 * Phase 14: Enterprise Knowledge, Document Intelligence & RAG (imported
 * from usmanclaude92-creator/Artify-Backend---Google-AI-Studio-, commit
 * 4a1d7cd). Adapted: the source suite's "AiOrchestrator prompt augmentation"
 * and "searchKnowledgeBase AI tool" sub-tests depended on that repo's own
 * Phase 12 AI Control Center (`aiOrchestrator`, `aiToolExecutor`) — this
 * repo's own, already-tested Phase 12 stack (server/ai/toolRegistry.ts,
 * server/ai/governance.ts) is the sole AI-governance layer here (see
 * server/services/copilot/CopilotService.ts's header comment) and does not
 * (yet) register a `searchKnowledgeBase` tool, so those two sub-tests are
 * not ported. The KNOWLEDGE_RETRIEVAL workflow-step sub-test is fully
 * self-contained (WorkflowEngine + KnowledgeService only) and is ported
 * unchanged.
 *
 * Verifies:
 ...
 * 5. Grounded Prompt Context Augmentation with verified citations
 * 6. Autonomous Workflow Engine KNOWLEDGE_RETRIEVAL step
 * 7. Document Versioning & Reindexing lifecycle
 */
...
In case of P1 outages, page on-call SRE immediately.
  ...
  describe("6. Autonomous Workflow Engine Integration", () => {
    it("executes KNOWLEDGE_RETRIEVAL step inside autonomous workflow engine", async () => {
      ...
      const context = execution?.context as {
        kbResult?: { count: number; results: Array<{ documentTitle: string }> };
      };
